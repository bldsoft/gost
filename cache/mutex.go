...
func NewDistrMutex(cache DistrCacheRepository, lockKey string, unlockTime time.Duration) *DistrMutex {
...
func (m *DistrMutex) TryLock() bool {
	err := m.cache.AddFor(m.lockKey, m.uniqueID, int32(m.unlockTime.Seconds()))
	if err != nil {
...
func (m *DistrMutex) updateLock() {
	for range m.ticker.C {
		...
		switch lockOwner {
		case me:
			err := m.cache.SetFor(m.lockKey, m.uniqueID, int32(m.unlockTime.Seconds()))
			if err != nil {
