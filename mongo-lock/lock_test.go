...
func setup(t *testing.T) *mongo.Collection {
	...
	var err error
	if testDb == nil {
		testDb, err = mongo.Connect(options.Client().ApplyURI(testDbURI))
...
func TestLockExclusive(t *testing.T) {
	...
	err = client.XLock(ctx, "resource2", "aaaa", LockDetails{})
	if err != nil {
		t.Error(err)
	}
	err = client.XLock(ctx, "resource3", "bbbb", LockDetails{})
	if err != nil {
		t.Error(err)
	}
	...
	err = client.XLock(ctx, "resource1", "zzzz", LockDetails{})
	if err != ErrAlreadyLocked {
		t.Errorf("err = %s, expected the lock to fail due to the resource already being locked", err)
	}
	...
	// then with same resource name and lockId can able to create
	err = client.XLock(ctx, "resource4", "cccc", LockDetails{TTL: 1})
	if err != nil {
		t.Error(err)
	}
	...
	err = client.XLock(ctx, "resource4", "cccc", LockDetails{})
	if err != nil {
...
func TestLockShared(t *testing.T) {
	...
	// Create some locks.
	err := client.SLock(ctx, "resource1", "aaaa", LockDetails{}, 10)
	if err != nil {
		t.Error(err)
	}
	err = client.SLock(ctx, "resource1", "bbbb", LockDetails{}, 10)
	if err != nil {
		t.Error(err)
	}
	err = client.SLock(ctx, "resource2", "bbbb", LockDetails{}, 10)
	if err != nil {
		t.Error(err)
	}
...
func TestLockMaxConcurrent(t *testing.T) {
	...
	// Create some locks.
	err := client.SLock(ctx, "resource1", "aaaa", LockDetails{}, 2)
	if err != nil {
		t.Error(err)
	}
	err = client.SLock(ctx, "resource1", "bbbb", LockDetails{}, 2)
	if err != nil {
		t.Error(err)
	}

	// Try to create a third lock, which will be more than maxConcurrent.
	err = client.SLock(ctx, "resource1", "cccc", LockDetails{}, 2)
	if err != ErrAlreadyLocked {
		t.Errorf("err = %s, expected the lock to fail due to the resource already being locked", err)
	}
}
...
func TestLockInteractions(t *testing.T) {
	...
	err = client.SLock(ctx, "resource1", "bbbb", LockDetails{}, -1)
	if err != ErrAlreadyLocked {
		t.Errorf("err = %s, expected the lock to fail due to the resource already being locked", err)
	}
	...
	err = client.SLock(ctx, "resource2", "aaaa", LockDetails{}, -1)
	if err != nil {
		t.Error(err)
	}
	err = client.XLock(ctx, "resource2", "bbbb", LockDetails{})
	if err != ErrAlreadyLocked {
		t.Errorf("err = %s, expected the lock to fail due to the resource already being locked", err)
	}
}
...
func TestUnlock(t *testing.T) {
	...
	// Unlock a shared
	err = client.SLock(ctx, "resource2", "bbbb", LockDetails{}, -1)
	if err != nil {
		t.Error(err)
	}
...
func TestUnlockOrder(t *testing.T) {
	...
	err = client.SLock(ctx, "resource4", "aaaa", LockDetails{}, -1)
	if err != nil {
		t.Error(err)
	}
	err = client.XLock(ctx, "resource3", "bbbb", LockDetails{})
...
func TestStatusTTLValue(t *testing.T) {
	...
	err := client.XLock(ctx, "resource1", "aaaa", LockDetails{TTL: 3600})
	if err != nil {
		t.Error(err)
	}
	...
	err = client.XLock(ctx, "resource3", "cccc", LockDetails{TTL: 1})
	if err != nil {
		t.Error(err)
	}
	...
	// Make sure we get back 0 for the TTL of the expired
	f = Filter{
		LockId: "cccc",
	}
	actual, err = client.Status(ctx, f)
...
func TestRenew(t *testing.T) {
	...
	err := client.SLock(ctx, "resource4", "cccc", LockDetails{TTL: 3600}, -1)
	if err != nil {
		t.Error(err)
	}
	...
	err = client.SLock(ctx, "resource4", "aaaa", LockDetails{TTL: 3600}, -1)
	if err != nil {
		t.Error(err)
	}
	err = client.XLock(ctx, "resource3", "bbbb", LockDetails{})
	...
	err = client.SLock(ctx, "resource2", "aaaa", LockDetails{TTL: 3600}, -1)
	if err != nil {
		t.Error(err)
	}
...
func TestRenewTTLExpired(t *testing.T) {
	...
	err = client.SLock(ctx, "resource4", "aaaa", LockDetails{TTL: 1}, -1)
	if err != nil {
		t.Error(err)
	}
...
func initLockStatusLocks(client *Client) (time.Time, error) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*60)
	defer cancel()
	// Create a bunch of different locks.
	aaaaDetails := LockDetails{
		Owner: "john",
		Host:  "host.name",
		TTL:   3600,
	}
	bbbbDetails := LockDetails{
		Owner: "smith",
	}
	ccccDetails := LockDetails{
		TTL: 7200,
	}
...
// most part).
func validateLockStatuses(actual, expected []LockStatus) error {
	...
	var actualSorted LockStatusesByCreatedAtDesc
	var expectedSorted LockStatusesByCreatedAtDesc
	actualSorted = actual
